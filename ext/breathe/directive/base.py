    ...
    def warn(self, raw_text, rendered_nodes=None):
        ...
        return [
            ...
            self.state.document.reporter.warning(
                raw_text, line=self.context["lineno"]
            ),
        ]
...
def create_warning(project_info, state, lineno, **kwargs):
    ...
    context = dict(lineno=lineno, tail=tail, **kwargs)
...
class BaseDirective(rst.Directive):
    ...
    def render(
        ...
        try:
            ...
            return format_parser_error(
                "doxygenclass", e.error, e.filename, self.state, self.lineno
            )
