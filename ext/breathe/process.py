...
class AutoDoxygenProcessHandle(object):
    def __init__(
        self, path_handler, run_process, write_file, project_info_factory
    ):
    ...
    def generate_xml(self, projects_source, doxygen_options):
        ...
        for project_name, file_structure in projects_source.items():
            ...
            auto_project_info = (
                self.project_info_factory.create_auto_project_info(
                    project_name, folder
                )
            )

            project_files[project_name] = ProjectData(
                auto_project_info, contents
            )
        ...
        for project_name, data in project_files.items():
            ...
            project_path = self.process(
                data.auto_project_info, data.files, doxygen_options
            )

            project_info = data.auto_project_info.create_project_info(
                project_path
            )

            self.project_info_factory.store_project_info_for_auto(
    ...
    def process(self, auto_project_info, files, doxygen_options):
        ...
        full_paths = map(
            lambda x: auto_project_info.abs_path_to_source_file(x), files
        )
        ...
        cfg = AUTOCFG_TEMPLATE.format(
            ...
            extra="\n".join(
                "%s=%s" % pair for pair in doxygen_options.items()
            ),
        )

        build_dir = self.path_handler.join(
            auto_project_info.build_dir(), "breathe", "doxygen"
        )
        ...
        self.run_process(
            "doxygen %s" % quote(cfgfile), cwd=build_dir, shell=True
        )
