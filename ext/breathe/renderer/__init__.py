...
class DoxygenToRstRendererFactory(object):
    def __init__(self, parser_factory, project_info):

        self.parser_factory = parser_factory
        self.project_info = project_info

    def create_renderer(
        self, node_stack, state, document, filter_, target_handler
    ):
...
def format_parser_error(
    name, error, filename, state, lineno, do_unicode_warning
):
    ...
    if do_unicode_warning:
        ...
        unicode_explanation = [
            nodes.paragraph("", "", nodes.Text(unicode_explanation_text))
        ]
